...
@pytest.mark.parametrize("param,session_key", [
...
def test_clear_session_selective_keys(client, param, session_key):
    # Set up all session data
    with client.session_transaction() as sess:
        for key, value in ALL_SESSION_KEYS.items():
            sess[key] = value

    # Call the route to clear only the target key
    response = client.get(f'/clear_session?{param}=true', follow_redirects=True)
    assert response.status_code in (200, 302)

    # Check that only the target key is cleared, others remain
    with client.session_transaction() as sess:
