...
def list():
    ...
    elif session.get('prompt_ids') and 'clear_prompts' not in request.args:
        filter_kwargs['prompt_ids'] = session.get('prompt_ids')
...
def export():
    ...
    # Session-based context selections
    response_ids = session.get('response_ids', [])
    story_ids = session.get('story_ids', [])
    template_ids = session.get('template_ids', [])
    prompt_ids = session.get('prompt_ids', [])

    # Build consistent filter_kwargs
    filter_kwargs = {
        'flagged_only': flagged_only,
        'sort': sort
    }
    ...
    # Add session-based selection filters (same priority as /list)
    if response_ids and 'clear_responses' not in request.args:
        filter_kwargs['response_ids'] = response_ids
...
#realised old version wasn't making good use of resposne_service
...
def view_prompt_responses():
    ...
    # Clear any existing response filters
    session.pop('response_ids', None)

    # Store prompt_ids in session for potential future use (optional)
    session['prompt_ids'] = [str(pid) for pid in prompt_ids]

    # Redirect to /list with prompt_id filter via session, not query param
